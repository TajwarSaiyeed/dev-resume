...
const EditEducationSection = () => {
  ...
  return (
    <div
      className={
        "border p-3 rounded-md flex flex-wrap justify-between gap-2 my-3 bg-blue-200"
      }
    >
      <div
        className={`
      ...
      <div className={"flex flex-col justify-start items-start w-full gap-2"}>
        {data.map(lan => (
          <Card key={lan.id} className={"w-full"}>
            <CardHeader>
              <CardTitle>{lan.institute}</CardTitle>
            </CardHeader>
            <CardContent
        ...
        <Button onClick={handleAddNewInstitute}>Add Institute</Button>
      </div>
    </div>
  );
};
